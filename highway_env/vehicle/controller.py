...
class TrajectoryVehicle(Vehicle):
    ...
    def act(self, action: dict = {
        ...
        if abs(target_angle) <= 3 * np.pi / 4:  # within 135 deg on each side
        ...
        #         self.target_steering_angle = self.trajectory_angles[self.trajectory_index]
    ...
    def steering_control(self, target_steering_angle: float) -> float:
        """
        target_point = (distance, steering angle)
